...
kratos_clone_statik_test_pc()
{
    ...
    scp ~/software/kratos_statik_cluster_ssh.zip sunethw@129.187.141.45:~/software/
    rm -f kratos_statik_cluster_ssh.zip
    echo "-------- Expanding archive..."
    ssh sunethw@129.187.141.45 "cd ~/software/ && if [ -d $kratos_name ]; then rm -rf $kratos_name; fi && unzip -q kratos_statik_cluster_ssh.zip && rm -f kratos_statik_cluster_ssh.zip && cp modules/kratos_configure.sh.orig $kratos_name/scripts/configure.sh && echo $(date "+%Y-%m-%d %H:%M:%S") > $kratos_name/clone_time_stamp.dat"
    cd $current_pwd
}

kratos_sync_statik_test_pc()
{
    echo "---- Synching kratos at $KRATOS_PATH ----"
    kratos_name=$(echo $KRATOS_PATH | rev | cut -d"/" -f1 | rev)
    echo "------- Synching applications"
    rsync -avurcz $KRATOS_PATH/applications/ sunethw@129.187.141.45:~/software/$kratos_name/applications/
    echo "------- Synching kratos"
    rsync -avurcz $KRATOS_PATH/kratos/ sunethw@129.187.141.45:~/software/$kratos_name/kratos/
    echo "------- Synching external_libraries"
    rsync -avucrz $KRATOS_PATH/external_libraries/ sunethw@129.187.141.45:~/software/$kratos_name/external_libraries/
    ssh sunethw@129.187.141.45 "echo $(date "+%Y-%m-%d %H:%M:%S") > ~/software/$kratos_name/clone_time_stamp.dat"
}
...
then
    ...
    alias kratos_copy_compile_configuration='cp -f ~/software/modules/configure.sh.orig $KRATOS_PATH/scripts/configure.sh'
    ...
    alias kratos_test_file='sh $KRATOS_PATH/scripts/run_test.sh'
